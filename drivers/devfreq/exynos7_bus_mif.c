				...
				struct devfreq_data_mif *data)
{
	...
	if (soc_is_exynos7420()) {
		...
		case 1 :
			...
			if (rev_num == 1 && subrev_num == 0) {
				dev_pm_opp_disable(dev, devfreq_mif_opp_list[0].freq);
				dev_pm_opp_disable(dev, devfreq_mif_opp_list[1].freq);
				dev_pm_opp_disable(dev, devfreq_mif_opp_list[2].freq);
				dev_pm_opp_disable(dev, devfreq_mif_opp_list[3].freq);
			} else {
				dev_pm_opp_disable(dev, devfreq_mif_opp_list[0].freq);
				dev_pm_opp_disable(dev, devfreq_mif_opp_list[1].freq);
			}
	...
	dev_pm_opp_disable(dev, devfreq_mif_opp_list[8].freq);
...
static int exynos7_devfreq_mif_probe(struct platform_device *pdev)
{
	...
	data->old_volt = dev_pm_opp_get_voltage(target_opp);
