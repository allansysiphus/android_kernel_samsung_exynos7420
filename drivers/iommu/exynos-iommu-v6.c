...
static int __sysmmu_pm_genpd_resume(struct device *dev) {
	int rc = 0;

#ifdef CONFIG_PM_RUNTIME
	rc = sysmmu_pm_genpd_resume(dev);
	if (rc)
		return rc;
#endif
#ifdef CONFIG_PM_SLEEP
	rc = sysmmu_pm_genpd_restore_state(dev);
#endif

	return rc;
}

static int __sysmmu_pm_genpd_suspend(struct device *dev) {
	int rc = 0;

#ifdef CONFIG_PM_RUNTIME
	rc = sysmmu_pm_genpd_save_state(dev);
	if (rc)
		return rc;
#endif
#ifdef CONFIG_PM_SLEEP
	rc = sysmmu_pm_genpd_suspend(dev);
#endif
...
static struct dev_pm_ops sysmmu_dev_pm_ops = {
	.resume = &__sysmmu_pm_genpd_resume,
	.suspend = &__sysmmu_pm_genpd_suspend,
};
					...
					void *p)
{
	...
	{
		if (dev->pm_domain) {
			dev->pm_domain->ops = sysmmu_dev_pm_ops;
		}
	...
	case BUS_NOTIFY_UNBOUND_DRIVER:
	{
		...
		dev->pm_domain->ops.suspend = NULL;
		dev->pm_domain->ops.resume = NULL;
