...
#define __max77843_charger_probe_define(_module_) \
...
static int max77843_charger_probe(struct platform_device *pdev)
{
	...
	struct power_supply_desc *curr_desc, *psy_chg_d, *psy_otg_d;
	struct power_supply_config *curr_cfg, *psy_chg_c, *psy_otg_c;
	int ret = 0;
	u8 reg_data;
	...
	__max77843_charger_probe_define(chg);
	psy_chg_d->name		= "max77843-charger";
	psy_chg_d->type		= POWER_SUPPLY_TYPE_UNKNOWN;
	psy_chg_d->get_property	= max77843_chg_get_property;
	psy_chg_d->set_property	= max77843_chg_set_property;
	psy_chg_d->properties	= max77843_charger_props;
	psy_chg_d->num_properties	= ARRAY_SIZE(max77843_charger_props);

	__max77843_charger_probe_define(otg);
	psy_otg_d->name		= "otg";
	psy_otg_d->type		= POWER_SUPPLY_TYPE_OTG;
	psy_otg_d->get_property	= max77843_otg_get_property;
	psy_otg_d->set_property	= max77843_otg_set_property;
	psy_otg_d->properties	= max77843_otg_props;
	psy_otg_d->num_properties	= ARRAY_SIZE(max77843_otg_props);

	charger_chip_name = psy_chg_d->name;
	max77843_charger_initialize(charger);
	...
	psy_tmp = power_supply_register(&pdev->dev, psy_chg_d, psy_chg_c);
	if (psy_tmp) {
		pr_err("%s: Failed to Register psy_chg\n", __func__);
		goto err_power_supply_register_chg;
	}
	charger->psy_chg = *psy_tmp;

	psy_tmp = power_supply_register(&pdev->dev, psy_otg_d, psy_otg_c);
	if (psy_tmp) {
		pr_err("%s: Failed to Register psy_otg\n", __func__);
		goto err_power_supply_register_otg;
	}
	charger->psy_otg = *psy_tmp;
