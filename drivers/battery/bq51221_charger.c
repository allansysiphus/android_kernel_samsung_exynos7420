						...
						const struct i2c_device_id *id)
{
	...
	struct power_supply_config *psy_chg_c;
	int ret = 0;
	...
	psy_chg_d = kzalloc(sizeof(*psy_chg_d), GFP_KERNEL);
	if (!psy_chg_d) {
		return -ENOMEM;
	}

	psy_chg_c = kzalloc(sizeof(*psy_chg_c), GFP_KERNEL);
	if (!psy_chg_c) {
		return -ENOMEM;
	}

	psy_chg_d->name		= pdata->wireless_charger_name;
	psy_chg_d->type		= POWER_SUPPLY_TYPE_UNKNOWN;
	psy_chg_d->get_property	= bq51221_chg_get_property;
	psy_chg_d->set_property	= bq51221_chg_set_property;
	psy_chg_d->properties	= sec_charger_props;
	psy_chg_d->num_properties	= ARRAY_SIZE(sec_charger_props);
	...
	psy_tmp = power_supply_register(&client->dev, psy_chg_d, psy_chg_c);
	if (!psy_tmp) {
