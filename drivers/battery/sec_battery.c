...
#define __sec_battery_probe_define(_module_) \
	psy_##_module_##_c = kzalloc(sizeof(*psy_##_module_##_c), GFP_KERNEL); \
	if (!psy_##_module_##_c) {\
		return -ENOMEM; \
	} \
	psy_##_module_##_d = kzalloc(sizeof(*psy_##_module_##_d), GFP_KERNEL); \
	if (!psy_##_module_##_d) {\
		return -ENOMEM; \
	} \
	curr_cfg = psy_##_module_##_c; \
	curr_desc = psy_##_module_##_d
...
static int sec_battery_probe(struct platform_device *pdev)
{
	...
	struct power_supply_desc *curr_desc, *psy_bat_d, *psy_usb_d, *psy_ac_d, *psy_wireless_d, *psy_ps_d;
	struct power_supply_config *curr_cfg, *psy_bat_c, *psy_usb_c, *psy_ac_c, *psy_wireless_c, *psy_ps_c;
	int ret = 0;
#ifndef CONFIG_OF
	int i;
#endif
	...
	// psy_bat
	__sec_battery_probe_define(bat);
	curr_desc->name = "battery";
	curr_desc->type = POWER_SUPPLY_TYPE_BATTERY;
	curr_desc->properties = sec_battery_props;
	curr_desc->num_properties = ARRAY_SIZE(sec_battery_props);
	curr_desc->get_property = sec_bat_get_property;
	curr_desc->set_property = sec_bat_set_property;

	// psy_usb
	__sec_battery_probe_define(usb);
	curr_desc->name = "usb";
	curr_desc->type = POWER_SUPPLY_TYPE_USB;
	curr_desc->properties = sec_power_props;
	curr_desc->num_properties = ARRAY_SIZE(sec_power_props);
	curr_desc->get_property = sec_usb_get_property;
	curr_cfg->supplied_to = supply_list;
	curr_cfg->num_supplicants = ARRAY_SIZE(supply_list);

	// psy_ac
	__sec_battery_probe_define(ac);
	curr_desc->name = "ac";
	curr_desc->type = POWER_SUPPLY_TYPE_MAINS;
	curr_desc->properties = sec_ac_props;
	curr_desc->num_properties = ARRAY_SIZE(sec_ac_props);
	curr_desc->get_property = sec_ac_get_property;
	curr_cfg->supplied_to = supply_list;
	curr_cfg->num_supplicants = ARRAY_SIZE(supply_list);

	// psy_wireless
	__sec_battery_probe_define(wireless),
	curr_desc->name = "wireless";
	curr_desc->type = POWER_SUPPLY_TYPE_WIRELESS;
	curr_desc->properties = sec_power_props;
	curr_desc->num_properties = ARRAY_SIZE(sec_power_props);
	curr_desc->get_property = sec_wireless_get_property;
	curr_desc->set_property = sec_wireless_set_property;
	curr_cfg->supplied_to = supply_list;
	curr_cfg->num_supplicants = ARRAY_SIZE(supply_list);
	
	// psy_ps
	__sec_battery_probe_define(ps);
	curr_desc->name = "ps";
	curr_desc->type = POWER_SUPPLY_TYPE_POWER_SHARING;
	curr_desc->properties = sec_ps_props;
	curr_desc->num_properties = ARRAY_SIZE(sec_ps_props);
	curr_desc->get_property = sec_ps_get_property;
	curr_desc->set_property = sec_ps_set_property;
	curr_cfg->supplied_to = supply_list;
	curr_cfg->num_supplicants = ARRAY_SIZE(supply_list);
	...
	psy_tmp = power_supply_register(&pdev->dev, psy_ps_d, psy_ps_c);
	if (!psy_tmp) {
		dev_err(battery->dev,
	...
	battery->psy_ps = *psy_tmp;

	psy_tmp = power_supply_register(&pdev->dev, psy_wireless_d, psy_wireless_c);
	if (!psy_tmp) {
		dev_err(battery->dev,
	...
	battery->psy_wireless = *psy_tmp;

	psy_tmp = power_supply_register(&pdev->dev, psy_usb_d, psy_usb_c);
	if (!psy_tmp) {
		dev_err(battery->dev,
	...
	battery->psy_usb = *psy_tmp;

	psy_tmp = power_supply_register(&pdev->dev, psy_ac_d, psy_ac_c);
	if (!psy_tmp) {
		dev_err(battery->dev,
	...
	battery->psy_ac = *psy_tmp;

	psy_tmp = power_supply_register(&pdev->dev, psy_bat_d, psy_bat_c);
	if (!psy_tmp) {
		dev_err(battery->dev,
