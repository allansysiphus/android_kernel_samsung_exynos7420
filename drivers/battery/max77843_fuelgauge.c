...
static int max77843_fuelgauge_probe(struct platform_device *pdev)
{
	...
	struct power_supply_desc *psy_fg_d;
	struct power_supply_config *psy_fg_c;
	int ret = 0;
	union power_supply_propval raw_soc_val;
#if defined(CONFIG_DISABLE_SAVE_CAPACITY_MAX)	
	u16 reg_data;
	...
	psy_fg_d = kzalloc(sizeof(*psy_fg_d), GFP_KERNEL);
	if (!psy_fg_d) {
		return -ENOMEM;
	}

	psy_fg_c = kzalloc(sizeof(*psy_fg_c), GFP_KERNEL);
	if (!psy_fg_c) {
		return -ENOMEM;
	}

	psy_fg_d->name		= "max77843-fuelgauge";
	psy_fg_d->type		= POWER_SUPPLY_TYPE_UNKNOWN;
	psy_fg_d->get_property	= max77843_fg_get_property;
	psy_fg_d->set_property	= max77843_fg_set_property;
	psy_fg_d->properties	= max77843_fuelgauge_props;
	psy_fg_d->num_properties = ARRAY_SIZE(max77843_fuelgauge_props);
	...
	psy_tmp = power_supply_register(&pdev->dev, psy_fg_d, psy_fg_c);
	if (!psy_tmp) {
		pr_err("%s: Failed to Register psy_fg\n", __func__);
		goto err_data_free;
	}
	fuelgauge->psy_fg = *psy_tmp;
