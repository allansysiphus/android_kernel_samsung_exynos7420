...
DEVICE_ATTR_RW(mode);
DEVICE_ATTR_RW(scenario);
DEVICE_ATTR_RW(tuning);
DEVICE_ATTR_RW(accessibility);
DEVICE_ATTR_RO(color_correct);
DEVICE_ATTR_RW(bypass);
DEVICE_ATTR_RW(lux);
DEVICE_ATTR_RO(mdnie);
DEVICE_ATTR_RW(sensorRGB);
DEVICE_ATTR_RW(night_mode);
DEVICE_ATTR_RW(mdnie_ldu);
#ifdef CONFIG_LCD_HMT
DEVICE_ATTR_RW(hmt_color_temperature);
#endif
