...
{
	const struct pinconf_ops *ops = pctldev->desc->confops;
	unsigned long configs[1] = { config };
	int ret;
			 ...
			 unsigned long config)
{
	...
	int ret;
	int i;
	unsigned long configs[1] = { config }; // really, really ugly workaround, but it works
	...
	if (ops->pin_config_group_set) {
		ret = ops->pin_config_group_set(pctldev, selector, configs, 1);
		/*
	...
	for (i = 0; i < num_pins; i++) {
		ret = pin_config_set_for_pin(pctldev, pins[i], config);
		if (ret < 0)
			goto unlock;
	}
